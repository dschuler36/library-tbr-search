...
export class StoryGraphParser implements CSVParser {
	getName(): string {
		return 'Storygraph';
	}
	...
	async parse(fileContent: string): Promise<Book[]> {
		return new Promise((resolve, reject) => {
			Papa.parse<StoryGraphRow>(fileContent, {
						...
						for (const row of results.data) {
							...
							if (parsed.success && parsed.data['Read Status'] === 'to-read') {
								books.push({
									...
									authors: parsed.data.Authors.trim().replace(/\s+/g, ' '),
				...
				error: (error: Error) => {
					reject(new Error(`Storygraph CSV parsing error: ${error.message}`));
				}
			});
		});
	}
