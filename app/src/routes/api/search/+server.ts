...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (!books || !Array.isArray(books)) {
			return json({ error: 'Invalid books array' }, { status: 400 });
		}
		...
		for (let i = 0; i < books.length; i++) {
			...
			try {
				...
				if (availability) {
					...
					console.log(`  ${isAvailable ? '✅' : '⏳'} ${availability.status} - ${availability.availableCopies}/${availability.totalCopies} copies`);
		...
		const availableCount = results.filter(r => r.libraryMatch?.status === 'AVAILABLE').length;
